...
function normalizePhoneNumber(phoneNumber) {
    ...
    try {
        ...
        if (cleaned.startsWith('03') && cleaned.length === 10) {
            ...
            result = (0, phone_1.default)(cleaned);
            ...
            if (result.isValid &&
                result.phoneNumber &&
                !cleaned.startsWith('+')) {
                for (const [code, { minLength, maxLength, country },] of Object.entries(knownCountryCodes)) {
                    if (cleaned.startsWith(code)) {
                        // Check if length is correct for this country code
                        const isValidLength = cleaned.length >= minLength &&
                            cleaned.length <= maxLength;
                        // Check if the matched country is wrong
                        const matchedWrongCountry = result.countryIso2 !== country &&
            ...
            if (!result.isValid &&
                ...
                if (afterCountryCode.length >= 8 &&
                    afterCountryCode.length <= 9 &&
                    ['2', '3', '4', '8', '9'].includes(afterCountryCode.charAt(0))) {
                    ...
                    if (!mobilePrefixes.some((prefix) => afterCountryCode.startsWith(prefix))) {
                        ...
                        const withPlus = cleaned.startsWith('+')
                            ? cleaned
                            : `+${cleaned}`;
                        const tempResult = (0, phone_1.default)(withPlus, { country: 'IL' });
                ...
                for (const code of knownCountryCodesList) {
                    if (cleaned.startsWith(code)) {
                        ...
                        if (codeInfo &&
                            ...
                            result = (0, phone_1.default)(withPlus);
                            if (result.isValid) {
                                break;
                            }
                        }
            ...
            if (!result.isValid) {
                if (cleaned.startsWith('0')) {
                    ...
                    for (const hint of countryHints) {
                        result = (0, phone_1.default)(cleaned, { country: hint });
                        if (result.isValid) {
                            break;
                        }
                    }
                    ...
                    if (!result.isValid) {
                        ...
                        for (const hint of countryHints) {
                            result = (0, phone_1.default)(withPlus, { country: hint });
                            if (result.isValid) {
                                break;
                            }
                        }
